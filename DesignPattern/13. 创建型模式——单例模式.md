...
### JavaScript 实现——通过辅助函数实现单例

使用闭包的方式，编写一个 `getSingle` 辅助函数，将任意类转换为单例类。无论调用多少次，始终返回同一个实例对象。
...
### TypeScript 实现——利用 private 控制构造函数可访问性

通过将构造函数设为 `private`，阻止外部直接 `new` 创建实例；通过静态方法 `getInstance` 提供全局唯一的访问点。
