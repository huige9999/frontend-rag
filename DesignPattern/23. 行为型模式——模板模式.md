...
### Coffee.ts —— 具体子类：冲泡咖啡，重写钩子方法决定不加调料
...
function prepareRecipe(beverage: Beverage) {
  beverage.init();
}
