...
定义所有产品需要遵循的统一接口，确保不同产品具有一致的调用方式。
...
工厂类通过静态方法 `createProduct`，根据传入的类型参数决定创建哪种具体产品。客户端无需关心具体的创建逻辑。
...
客户端只需引入工厂类，通过工厂的统一接口获取产品实例，无需直接依赖具体的产品类。
