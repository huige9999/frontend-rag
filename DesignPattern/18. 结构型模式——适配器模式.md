...
来自第三方的功能类，只接受 JSON 格式的数据进行分析，接口与客户端不兼容。

```typescript
// 服务类，提供了针对 JSON 数据格式的分析方法

export class Service {
  public serviceMethod(data: string): void {
    console.log(`分析 JSON 格式的数据：${data}`);
  }
}
```
...
作为客户端与服务类之间的桥梁，负责将数据格式从 XML 转换为 JSON，然后调用服务类的方法。
...
客户端通过适配器间接调用第三方服务，无需关心数据格式转换的细节。
