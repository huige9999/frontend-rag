...
### JavaScript 封装示例（index.js）

使用 ES2020 的 `#` 私有字段语法实现封装，通过 getter/setter 控制对私有属性的访问：
...
**要点说明：**
- `#name` 和 `#age` 是真正的私有字段，类外部无法直接访问
- `#privateMethod()` 是私有方法，同样只能在类内部调用
- 通过 `get name()` / `set name()` 提供受控的访问接口

---

### TypeScript 封装示例（index.ts）

使用 TypeScript 的 `private` 关键字实现封装，并在 setter 中添加校验逻辑：
...
**要点说明：**
- `private` 关键字修饰的成员只能在类内部访问
- getter/setter 命名用 `name`/`age`，私有字段用 `_name`/`_age` 前缀，避免命名冲突
- `set age()` 中包含校验逻辑：年龄大于 100 时输出提示并重置为默认值 20
