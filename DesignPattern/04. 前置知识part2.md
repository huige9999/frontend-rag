...
### TypeScript 继承示例（index.ts）

使用 `extends` 实现类的继承，子类 Student 继承父类 Person 的属性和方法，并扩展自己的属性和方法：
...
**要点说明：**
- 子类 `Student` 通过 `extends Person` 继承父类
- 子类构造函数中必须通过 `super(name, age)` 调用父类构造函数
- 子类可以调用父类的 `sayHello()` 方法，也可以定义自己的 `sayScore()` 方法
- 父类的 getter `get name()` 也可以被子类实例访问

---

### TypeScript 多态示例（index2.ts）

通过继承和方法重写实现多态，并结合鸭子类型特性，普通对象也能通过类型检查：
...
**要点说明：**
- `Dog` 和 `Cat` 都继承自 `Animal`，并重写了 `sayHello()` 方法，这是经典的子类多态
- `playSound` 函数接受 `Animal` 类型的参数，不同子类对象表现出不同的行为
- `obj` 是一个普通对象字面量（没有继承 Animal），但因为具有 `sayHello()` 方法，TS 的鸭子类型机制允许它通过类型检查，体现了 TS 灵活的多态特性
