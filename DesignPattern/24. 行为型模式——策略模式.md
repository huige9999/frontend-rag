...
#### Strategy.ts —— 具体策略类，分别实现加、减、乘、除四种算法
...
#### Context.ts —— 上下文类，维护策略引用并提供统一的执行接口
...
#### Interface.ts —— 出行策略接口，定义计算交通费用的方法
...
#### Strategy.ts —— 具体出行策略，分别实现公交、出租车、地铁、共享单车的费用计算
...
export default class Context {
  // 维护具体的策略
  private strategy: ITravelStrategy;

  // 初始化策略
  constructor(st: ITravelStrategy) {
    this.strategy = st;
  }

  // 设置新的策略
  setStrategy(st: ITravelStrategy) {
    this.strategy = st;
  }

  goToAirport(distance: number): number {
    return this.strategy.calcFee(distance);
  }
}
```

#### Client.ts —— 客户端代码，选择公交策略计算去机场的费用

```typescript
import { BusStrategy, TaxiStrategy, SharedBicycleStrategy } from "./Strategy";
import Context from "./Context";

// 先实例化几种策略
const busStrategy = new BusStrategy();
const taxiStrategy = new TaxiStrategy();
const sharedBicycleStrategy = new SharedBicycleStrategy();

// 创建上下文，指定默认的策略
const context = new Context(busStrategy);
console.log(context.goToAirport(10)); // 6
```
