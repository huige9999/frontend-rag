...
模型"认为"——"它"更可能指的是"猫"，那么模型可能就会生成诸如"它吓倒了狗"的后续内容。

### 核心机制

当前词会去关注其它词，并且根据相关性分配权重，从而形成更有语境感知的一个表达。

## 多头注意力

### 定义

多头注意力指的是模型不只使用一个注意力机制去"看"上下文，而是使用多个注意力头，每个从不同的角度并行地关注输入中的不同部分。

### 各个头的关注点

一个注意力头只关注一套特定的关系，但现实语言中的语义很复杂：
...
自注意力机制让每个词元在上下文中"看别人"，整合其他词的信息。但仅靠这些信息还不够，Transformer 还需要进一步让每个词**自己思考、提炼、变换**。这个任务，就交给了前馈神经网络层。
...
前馈层也是非常重要的一层，看起来只是"对每个词做了一遍变换"，但它非常关键。如果 Transformer 只有注意力层没有前馈层，那它就像是一个只会模仿别人的人，不会自己总结、推理和表达。
