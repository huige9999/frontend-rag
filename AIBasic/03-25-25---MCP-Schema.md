...
MCP 里面提供了一组 Schema. Schema 由 TypeScript + Zod 定义，制作成 JSON Schema，用于**验证协议消息结构**。
...
- **Zod** 是强大的 TypeScript 模式验证库，提供类型安全和运行时校验
- **MCP Schema** 基于 Zod 构建，为 JSON-RPC 协议提供结构校验
- **setRequestHandler** 是注册请求处理器的核心方法，自动处理校验和响应封装
- 通过 Schema 和 Handler 的配合，可以构建类型安全的 MCP Server
