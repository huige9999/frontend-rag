...
网站首页的布局，是由 docs 下面的 index.md 来决定。
...
首先第一步，你需要有一些文档，因为我们之前配置了 docs 作为我们的文档目录，因此我们需要将所有的文档有层次的放入到 docs 里面。
...
VitePress 文档项目的依赖配置文件，定义了开发、构建和预览脚本。
...
VitePress 核心配置文件，配置了网站标题、语言、导航栏、侧边栏和社交链接。
...
网站首页配置，使用 frontmatter 定义首页的 hero 区域和 features 展示。
