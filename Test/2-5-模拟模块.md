...
用户 API 模块，封装了 axios 请求，提供 all 方法获取所有用户数据。
...
测试文件，使用 jest.mock 模拟 axios 第三方模块，通过工厂函数指定模拟行为，同时验证自定义的 testArg 方法。
...
### test/mockFile.test.js

测试文件，使用 jest.mock 模拟文件模块，通过 jest.requireActual 获取原始模块并部分替换方法。
