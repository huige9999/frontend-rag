...
根据 checkbox 勾选状态控制子内容显示/隐藏的 React 组件。
...
登录表单组件，使用 useReducer 管理状态，通过 fetch 发送登录请求并处理成功/失败响应。
...
### HiddenMessage.test.js

测试 HiddenMessage 组件：验证默认不显示内容，勾选后内容出现。
...
使用 msw 模拟服务器，测试 Login 组件的请求成功和请求失败两种场景。
...
Jest 测试环境初始化文件，引入 jest-dom 扩展断言匹配器。
...
项目依赖配置，包含 React、Testing Library 及 msw 等依赖。
