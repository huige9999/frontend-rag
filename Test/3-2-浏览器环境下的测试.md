...
Jest 配置文件，核心配置为 `testEnvironment: "jsdom"`，用于模拟浏览器环境。
...
localStorage 工具模块，封装了带前缀 key 的存储和获取方法。
...
工具函数库，包含 `getSearchObj`（URL查询字符串转对象）和 `fetchData`（fetch请求数据）两个函数。
...
### __test__/storage.test.ts

测试 storage 模块的存储和获取功能。
...
### __test__/getSearchObj.test.ts

测试 `getSearchObj` 函数，使用 jest-location-mock 模拟浏览器 location 对象。
...
### __test__/fetchData.test.ts

测试 `fetchData` 函数，使用 jest-fetch-mock 模拟浏览器 fetch API。
