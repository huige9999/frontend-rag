...
项目依赖配置，包含 jest-haste-map（文件查找）、jest-worker（并行执行）、expect（断言）、jest-mock（模拟）、jest-circus（describe/it 支持）、chalk（输出美化）等核心依赖。
...
测试框架主入口，负责通过 jest-haste-map 获取测试文件、使用 jest-worker 并行调度执行、通过 chalk 美化输出测试结果。
...
工作线程模块，在子线程中执行测试文件代码，包含多个演进版本（从手动实现 expect 到使用 jest-circus 支持 describe/it）。
...
### tests/01.test.js ~ 06.test.js

基础断言测试文件，用于验证 expect().toBe() 匹配器的正确与失败场景。其中 01、03、05 为失败用例（值不匹配），02、04、06 为成功用例。
...
测试 describe/it 语法支持，验证 jest-circus 集成后的分组测试能力。
...
测试 jest-mock 的 fn() 模拟函数功能，验证 toHaveBeenCalled 断言。
