...
Jest 配置文件，使用 jsdom 环境以支持 window 对象上的计时器方法。
...
计时器工具函数模块，提供 `startTimer`/`stopTimer`（setInterval）和 `startTimeout`/`stopTimeout`（setTimeout）两组方法，并包含负数参数的边界处理。
...
测试 setInterval 相关的 `startTimer` 和 `stopTimer` 函数，使用 fake timers 和 jest.spyOn 模拟计时器。
...
测试 setTimeout 相关的 `startTimeout` 和 `stopTimeout` 函数。
