...
### HiddenMessage.test.ts

测试 HiddenMessage 组件的正确渲染、默认隐藏状态以及勾选后显示/隐藏信息的功能。
...
使用 jest.fn 模拟 fetch 请求，测试登录组件的请求成功和请求失败两种场景。
...
测试 ToDoList 组件的新增待办事项功能，并包含快照测试。
...
Jest 配置文件，设置 TypeScript + Babel 预设并配置 testMatch 匹配 .test 和 .spec 文件。
...
项目依赖配置，包含 Vue 3、Vue Test Utils、Jest 及 TypeScript 等依赖。
...
根据 checkbox 勾选状态控制插槽内容显示/隐藏的 Vue 组件。
...
登录表单组件，通过 fetch 发送登录请求并根据响应状态更新界面。
