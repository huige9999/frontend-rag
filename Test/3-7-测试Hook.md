...
自定义计数器 Hook，支持增减、设置、重置操作，并支持 min/max 边界限制和异步增加。
...
使用 renderHook 和 act 测试 useCounter Hook 的加法、减法、设置值、重置、边界限制及异步增加。
...
项目依赖配置，包含 React、Testing Library 及 TypeScript 等依赖。
