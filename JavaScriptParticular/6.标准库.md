...
var user1 = { name: "asfd", age: 123 };
var user2 = { name: "546345", age: 11 };

// bind：返回新函数，this始终指向user1
...
年（year）、月（month）、日（date）、小时（hour）、分钟（minute）、秒（second）= 1000ms、毫秒（ms）= 1000 us、微秒（us）= 1000 ns、纳秒（ns）
...
UTC和GMT之间误差不超过0.9秒。GMT+0800 表示东8区。

3. 时间戳

1970-1-1 凌晨到某个时间所经过的毫秒数
...
注意：月份的数字从0开始计算。如果缺失参数，日期部分默认为1，时分秒毫秒默认为0。

## 实例成员

**获取方法：** getDate、getDay（0表示星期天）、getFullYear、getHours、getMinutes、getSeconds、getMilliseconds、getTime（时间戳）、getMonth（从0开始）

**设置方法：** setDate、setMonth、setFullYear、setMinutes、setSeconds、setMilliseconds、setTime

**格式化方法：** toDateString、toISOString、toLocaleDateString、toLocaleString、toLocaleTimeString

## 日期的运算

日期对象重写了Object中的valueOf方法，返回的是一个数字，表示时间戳，因此日期对象可以进行数学运算。
...
正则表达式是国际标准，跨越语言。正则表达式是一个规则，用于验证字符串。

## 基础

1. 字面量匹配：规则中直接书写字面量字符
...
1. 字面量模式：`/pattern/flags`
2. 构造函数模式：`new RegExp("pattern", "flags")`
...
用小括号包裹的部分叫做捕获组，捕获组会出现在匹配结果中。捕获组可以命名，叫做具名捕获组。非捕获组使用`(?:...)`。
...
当运行代码1的时候，如果发生错误，立即停止代码1的执行，转而执行代码2，错误对象为抛出的错误对象。无论代码1和代码2是否出错，最终都将执行代码3。
