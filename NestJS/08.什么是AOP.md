...
![image-20250115162906030](./assets/image-20250115162906030.png)
...
![Nest请求流程](./assets/image-20250115165534832.png)
...
通过 TypeScript 方法装饰器，模拟 AOP 的三种通知类型：

**Before 装饰器** — 在原方法执行前插入逻辑：
...
exampleService.greet("jack");
```

> 三个装饰器的核心思路一致：通过修改 `descriptor.value`，将原方法包装一层，在调用前后插入额外逻辑，从而实现不侵入业务代码的横切关注点处理。
