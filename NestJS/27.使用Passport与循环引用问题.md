...
### 1. 循环引用解决方案 -- forwardRef

当 AuthModule 和 UserModule 互相引用时，使用 `forwardRef` 延迟加载模块引用，避免初始化时的循环依赖：

**user.module.ts** -- 导出 UserService 并用 forwardRef 引入 AuthModule：
...
**auth.module.ts** -- 导入 PassportModule、JwtModule，用 forwardRef 引入 UserModule：
...
@Module({
  imports: [
    forwardRef(() => UserModule),
    PassportModule,
    JwtModule.register({ secret: 'MySecret', signOptions: { expiresIn: '7d' } }),
  ],
  providers: [AuthService, LocalStrategy, JwtStrategy],
  exports: [AuthService],
})
...
### 2. 本地认证策略 -- LocalStrategy

继承 `PassportStrategy(Strategy)`，在 `validate` 方法中调用 AuthService 验证用户名密码：
...
### 3. JWT认证策略 -- JwtStrategy

从请求头 Bearer Token 中提取 JWT，验证后返回 payload 中的用户信息：
...
### 4. AuthService -- 验证与签发Token

`validateUser` 验证用户名密码，`login` 签发 JWT：
...
### 5. Controller中使用AuthGuard

通过 `@UseGuards(AuthGuard('local'))` 拦截登录请求，通过 `@UseGuards(AuthGuard('jwt'))` 保护需要认证的路由：
...
### 6. 环境变量配置 -- ConfigModule + forRootAsync

在 AppModule 中全局注册 ConfigModule，使用工厂函数动态注入配置：
