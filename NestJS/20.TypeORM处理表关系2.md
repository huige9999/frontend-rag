...
配置TypeORM与MySQL的连接，`synchronize: true` 会自动同步实体定义到数据库表结构，`entities` 使用通配符路径自动加载所有实体文件。
...
User 实体中通过 `@OneToOne` 关联 IdCard，并设置 `cascade: true` 实现级联保存/更新：
...
关联查询时通过 `relations` 选项指定要加载的关联关系：

```typescript
// 从User端查询关联的IdCard
const users = await userRepository.find({ relations: { card: true } });

// 级联更新：修改user及其关联的card后，一次save即可同时更新两表
const user = await userRepository.findOne({ relations: { card: true }, where: { id: 1 } });
user.name = 'lucy';
user.card.name = 'lucy';
await userRepository.save(user);
```

### 一对多/多对一关系 - Department 与 Employee

`@OneToMany` 必须与 `@ManyToOne` 配对使用，外键列设置在"多"的一方（Employee）：
...
// Employee.ts - "多"的一方，持有外键
@ManyToOne(() => Department, department => department.employees)
@JoinColumn()
department: Department;
```

插入数据时，将员工数组赋给部门的 employees 属性，一次 save 即可级联保存：
...
### 多对多关系 - Order 与 Product

使用 `@ManyToMany` 和 `@JoinTable` 自动创建中间表。`@JoinTable` 只需加在其中一方即可：
...
// Product.ts - 通过 @JoinTable 自定义中间表名称和列名
@ManyToMany(() => Order, order => order.products)
@JoinTable({
  name: "order_products",
...
插入数据时，将商品数组赋给订单的 products 属性，保存订单时自动级联保存商品和中间表记录：
...
在 `app.module.ts` 中通过 `TypeOrmModule.forRoot()` 注册全局数据库连接，在业务模块中通过 `TypeOrmModule.forFeature([User])` 注册实体存储库，然后在 Service 中通过 `@InjectRepository(User)` 注入 Repository 使用。
