...
使用`EntityManager`的小问题：每个方法都需要传入实体类作为第一个参数。
...
项目代码位于 `17.TypeORM的基本使用/代码/typeorm-test/` 目录，包含三个核心文件。

### 1. 数据源配置 (data-source.ts)

通过 DataSource 配置 MySQL 连接参数，`synchronize: true` 可自动根据实体同步建表，`logging: true` 打印生成的 SQL 语句。`entities` 数组注册所有实体类。
...
### 2. 用户实体定义 (entity/User.ts)

使用 `@Entity()` 将类标记为数据库实体，`@PrimaryGeneratedColumn()` 定义自增主键，`@Column()` 定义普通列。可通过装饰器参数指定列类型、长度和默认值。
...
### 3. CRUD 操作示例 (index.ts)

调用 `AppDataSource.initialize()` 建立连接后，使用 EntityManager 的 `save`、`find`、`findOneBy` 方法完成增查改操作。核心逻辑如下：
