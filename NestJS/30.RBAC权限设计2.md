...
使用`entityManager.findOne`方法查询User信息，并且通过`relations`关联查出Role角色的信息。
...
两个模块都有对应的controller，现在都能进行简单的访问。
...
3、创建permissionGuard守卫，通过用户拥有权限与路由装饰器规定权限进行匹配，如果成功，则放行
...
### 1. 自定义装饰器 - custom-decorator.ts

通过 `SetMetadata` 定义两个自定义装饰器，分别用于标记需要登录和需要特定权限的路由：
...
### 2. 实体设计 - User / Role / Permission

三个实体通过 `ManyToMany` + `JoinTable` 建立多对多关系：

```typescript
// user.entity.ts - 用户与角色多对多
@ManyToMany(() => Role)
@JoinTable({ name: 'user_role_relation' })
roles: Role[];

// role.entity.ts - 角色与权限多对多
@ManyToMany(() => Permission)
...
核心逻辑：通过 Reflector 读取 metadata 判断是否需要登录，不需要则直接放行；需要登录则解析 JWT token 并将用户信息挂载到 request 上供后续守卫使用：
...
export class LoginGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext) {
    ...
    try {
      ...
      // 将用户信息挂载到 request 上，供 PermissionGuard 使用
      (request as any).user = data.user;
      return true;
    } catch (e) {
...
核心逻辑：从 request 上获取用户角色 -> 查询角色拥有的全部权限 -> 与路由要求的权限逐个比对：
...
export class PermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 4. 逐个比对：缺少任一权限则拒绝
    for (let i = 0; i < permissionRequired.length; i++) {
      const found = permissions.find((item) => item.name === permissionRequired[i]);
...
### 5. UserService - 角色权限查询

`findRoleByIds` 通过 `In` 操作符批量查询角色，并关联查出权限：
...
### 6. 全局守卫注册 - app.module.ts

在 AppModule 中通过 `APP_GUARD` 注册两个全局守卫，执行顺序为 LoginGuard -> PermissionGuard：
...
### 7. Controller 装饰器使用示例

在 Controller 类上用 `@LoginRequired()` 标记需要登录，在具体路由方法上用 `@PermissionRequired()` 标记所需权限：
