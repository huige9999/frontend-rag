...
应用启动入口，通过 `NestFactory.create()` 创建应用实例并监听端口：
...
通过 `@Module()` 装饰器组织应用结构，`imports` 引入子模块，`controllers` 和 `providers` 注册控制器和服务：
...
使用 `@Controller('app')` 定义路由前缀，`@Get` / `@Post` 定义请求方法，Controller 负责接收请求并调用 Service 处理：
...
使用 `@Injectable()` 装饰器标记为可注入服务，封装业务逻辑和数据处理：
...
### nest g resource 生成的 CRUD 模板

通过 `nest g resource person` 一键生成的 REST 风格 CRUD 代码结构，Controller 中包含了完整的增删改查路由：
