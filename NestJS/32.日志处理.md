...
Logger模块通过`providers`注册MyLogger，通过`exports`导出以供其他模块注入使用。
...
### MyLogger - 自定义Winston日志器（核心代码）

MyLogger实现`LoggerService`接口，内部使用Winston创建日志器，配置了三种传输方式：控制台输出（带颜色格式化）、按日滚动的info级别日志文件、按日滚动的error级别日志文件。

```typescript
// src/logger/MyLogger.ts
@Injectable()
export class MyLogger implements LoggerService {
  ...
  // info / warn / debug 方法同理，用dayjs格式化时间戳后调用对应的winston方法
}
```

### main.ts - 应用启动配置

关闭内置日志器（`logger: false`），使用自定义的MyLogger替换。
...
通过依赖注入使用MyLogger，在业务方法中记录不同级别的日志。
