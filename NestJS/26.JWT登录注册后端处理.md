...
### 1. User 实体（entities/user.entity.ts）

定义用户表结构，包含用户名、密码、创建时间和更新时间：
...
### 2. DTO 数据验证（dto/register-user.dto.ts / dto/login-user.dto.ts）

注册 DTO 使用 class-validator 进行严格校验（长度、正则匹配），登录 DTO 仅校验非空，体现两个场景不同的验证需求：
...
export class RegisterUserDto {
  ...
  @IsNotEmpty()
  @Length(6, 20)
  @Matches(/^[A-Za-z0-9_-]+$/, { message: '用户名只能包含字母、数字、下划线和破折号' })
  username: string;
...
### 3. UserService 业务逻辑（user.service.ts）

核心业务：注册时检查用户名是否已存在并用 md5 加密密码，登录时验证用户名和密码：
...
export class UserService {
  ...
  async register(user: RegisterUserDto) {
    const userEntity = await this.userRepository.findOneBy({ username: user.username });
    if (userEntity) {
...
### 4. UserController 控制器（user.controller.ts）

登录成功后通过 JwtService 签发 token 并写入响应头，受保护路由使用 LoginGuard 守卫：
...
export class UserController {
  ...
  async login(
    ...
    if (result) {
      ...
      res.header('Authorization', token);
      return { message: '登录成功', data: result, code: 200 };
    } else {
      return { message: '登录失败', code: 400, data: null };
    }
  }
...
### 5. LoginGuard 登录守卫（login.guard.ts）

实现 CanActivate 接口，从请求头中提取并验证 JWT token，验证通过后将用户信息挂载到 request 对象上：
...
### 6. AppModule 模块配置（app.module.ts）

整合 TypeORM 数据库连接和 JwtModule 全局注册：
...
### 7. UserModule 模块注册（user.module.ts）

引入 TypeOrmModule.forFeature 注册 User 实体：
