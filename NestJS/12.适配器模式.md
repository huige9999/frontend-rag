...
### 1. 适配器模式示例（adapter-test/test.ts）

通过统一的 `Quackable` 接口和 `AnimalAdapter` 适配器，让不同类型的动物（鸭和鸡）都能使用同一个 `makeItQuack` 方法。
...
makeItQuack(adapter); // 输出: 小鸡 咯咯咯
```

### 2. Nest 切换 Fastify 适配器（fastify-test/src/main.ts）

核心变化：将默认的 Express 适配器替换为 `FastifyAdapter`，同时应用类型变为 `NestFastifyApplication`。
...
const app = await NestFactory.create<NestFastifyApplication>(
  AppModule,
  new FastifyAdapter(),  // 传入 Fastify 适配器即可切换底层框架
);
```

### 3. Fastify 下的 Controller（fastify-test/src/app.controller.ts）

切换到 Fastify 后，`@Req()` 和 `@Res()` 的类型从 Express 的 `Request`/`Response` 变为 Fastify 的 `FastifyRequest`/`FastifyReply`。
...
**说明：** 这正是适配器模式的实际应用 -- Nest 通过适配器层抽象了底层 HTTP 框架的差异，使得上层代码（Module、Service 等几乎不变）只需在入口处切换适配器，就能从 Express 平滑迁移到 Fastify。Controller 中需要将 Express 类型替换为 Fastify 对应类型。
