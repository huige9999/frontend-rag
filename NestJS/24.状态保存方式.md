...
本节课附带的示例代码是一个简单的 HMAC-SHA256 签名函数，用于演示信息签名的基本原理（与 JWT 的签名机制相关）：
...
- 使用 Node.js 内置的 `crypto` 模块，通过 HMAC-SHA256 算法对数据进行签名。
- `sign(info, secret)` 接收待签名内容和密钥，返回十六进制摘要字符串。
- 该机制与 JWT 中使用密钥对 Token 进行签名的原理一致。
