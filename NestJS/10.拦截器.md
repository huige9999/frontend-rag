...
### 1. TimeoutInterceptor -- 简单拦截器（记录请求URL）
...
通过 `ExecutionContext` 获取请求对象，在路由处理前打印请求URL，然后调用 `next.handle()` 放行。

### 2. AuthInterceptor -- 使用RxJS操作符的拦截器
...
export class AuthInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next.handle().pipe(
      map((data) => data.toUpperCase()),       // 响应数据转大写
      filter((data) => data.includes('A')),    // 过滤包含A的数据
...
在 `next.handle()` 返回的 Observable 上使用 `map`、`filter`、`tap`、`toArray`、`catchError` 等RxJS操作符，对响应数据进行链式转换处理。

### 3. PersonController -- 拦截器在路由方法上的使用
...
findName() {
  ...
  return from(['hello', 'worldA', 'abc']);  // 返回Observable数据流
}
```

通过 `@UseInterceptors` 装饰器将 `AuthInterceptor` 绑定到 `findName` 方法，方法返回 `from(['hello', 'worldA', 'abc'])` 形成Observable数据流供拦截器中的操作符处理。

### 4. PersonGuard -- 守卫（支持依赖注入）
...
守卫通过 `@Inject` 注入 `PersonService`，在 `canActivate` 中返回 `true` 放行或 `false` 拒绝（返回403）。

### 5. AppModule -- 通过IoC容器全局注册守卫和拦截器
...
使用 `APP_GUARD` 和 `APP_INTERCEPTOR` token 在IoC容器中全局注册，确保守卫和拦截器可以正常使用依赖注入。

### 6. PersonMiddleware -- 支持依赖注入的中间件
...
中间件通过 `@Inject` 注入服务，在路由处理前后打印日志。

### 7. PersonModule -- 模块中配置中间件
...
在模块中通过 `configure` 方法将中间件绑定到指定路由，模块需要实现 `NestModule` 接口。
