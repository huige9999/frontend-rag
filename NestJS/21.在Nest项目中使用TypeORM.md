...
以下为本课 Nest 项目中的核心代码片段。

### 1. AppModule -- TypeORM 数据库连接配置

通过 `TypeOrmModule.forRoot()` 在根模块注册数据库连接。
...
### 2. User Entity -- 用户实体定义

使用 `@Entity` 定义实体，`@PrimaryGeneratedColumn` 自增主键，`@Column` 定义普通列，`transformer` 处理日期类型转换。
...
### 3. UserModule -- 模块注册 Repository

使用 `TypeOrmModule.forFeature([User])` 在模块级别注册 Repository，这是使用 `@InjectRepository` 的前提。
...
### 4. UserService -- 三种数据库操作方式对比

Service 中展示了 `@InjectRepository` 注入 Repository 的 CRUD 操作，注释中保留了 `@InjectEntityManager` 的写法供对比。`findAll` 方法使用 QueryBuilder 实现 OR 条件查询。

```typescript
// src/user/user.service.ts
@Injectable()
export class UserService {
  // 方式一：@InjectEntityManager() 注入 EntityManager（注释部分）
  // 方式二：@InjectRepository(User) 注入 Repository（推荐）
  @InjectRepository(User)
  private userRepository: Repository<User>;
  ...
  async findAll(name: string, sex: string) {
    // Repository.find() 的 where 是 AND 连接
    // 使用 QueryBuilder 实现 OR 条件查询
    return this.userRepository
      .createQueryBuilder('u')
      .where('u.name = :name or u.sex=:sex', { name, sex })
      .getMany();
...
### 5. UserController -- RESTful 路由设计

标准的 Nest CRUD 控制器，通过 `@Query` 接收查询参数实现条件查询。
