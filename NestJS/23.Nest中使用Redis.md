...
### RedisModule -- Redis客户端模块封装

通过工厂函数创建Redis客户端并以 `REDIS_CLIENT` 为token注册为Provider，导出后供其他模块注入使用。
...
使用TypeORM定义购物车实体，`cartData` 字段以JSON类型存储购物车商品数量。

```typescript
// src/shopping-cart/entities/shopping-cart.entity.ts
@Entity()
export class ShoppingCart {
  @PrimaryGeneratedColumn()
  id: number;
...
### ShoppingCartService -- 核心业务逻辑（Redis + MySQL双写）

关键逻辑说明：
- **create**: 先写入MySQL，再写入Redis，保证双写一致性
- **findOne**: 先查Redis缓存，命中直接返回；未命中则查MySQL（典型的缓存旁路策略 Cache-Aside）
- **update**: 更新MySQL后同步更新Redis，并设置 `EX: 30` 缓存过期时间
...
export class ShoppingCartService {
  ...
  async update(updateShoppingCartDto: UpdateShoppingCartDto) {
    const { userId, cartData: { count = 1 } } = updateShoppingCartDto;
    const cartEntity = await this.findOne(userId);
    const cart = cartEntity ? cartEntity.cartData : {};
    cart.count = (cart.count || 0) + count;
    await this.shoppingCartRepository.update({ userId }, cartEntity);
    await this.redisClient.set(`cart:${userId}`, JSON.stringify(cartEntity), {
      EX: 30,
    });
    return { message: '更新购物车成功', success: true };
  }
}
```

### ShoppingCartModule -- 模块导入配置

必须同时导入 `RedisModule`（提供Redis客户端注入）和 `TypeOrmModule.forFeature([ShoppingCart])`（提供Repository注入）。
...
@Controller('shopping-cart')
export class ShoppingCartController {
  @Post()              // POST /shopping-cart        -- 添加购物车
  @Get(':userId')      // GET  /shopping-cart/:userId -- 查询购物车
  @Patch()             // PATCH /shopping-cart        -- 更新购物车
}
```
