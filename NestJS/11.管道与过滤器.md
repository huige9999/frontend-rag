...
**[RxJS](https://rxjs.dev/)**是一个用于处理异步数据流的JavaScript库。你可以把它理解为一个管道，它可以帮你更方便的处理各种事件和数据流，当然我们没必要增加心智负担，可以更直白的认为它就是和前端的`Lodash`工具库差不多的一个库。
...
Pipe 需要实现 `PipeTransform` 接口，并实现 `transform` 方法
...
实现 `PipeTransform` 接口，对参数进行类型验证和转换。当参数无法转为数字时抛出 `BadRequestException`。
...
### 自定义异常过滤器 - MyExceptionFilter

通过 `@Catch(HttpException)` 拦截 HTTP 异常，统一组装错误响应格式（包含 statusCode、message、path、success 字段）。

```typescript
// src/my-exception.filter.ts
@Catch(HttpException)
export class MyExceptionFilter implements ExceptionFilter {
  catch(exception: HttpException, host: ArgumentsHost) {
    const ctx = host.switchToHttp();
...
### 拦截器 - TimeoutInterceptor 与 AuthInterceptor

**TimeoutInterceptor**：简单的日志拦截器，通过 `ExecutionContext` 获取请求信息。
...
**AuthInterceptor**：利用 RxJS 操作符对响应数据进行流式处理（转大写、过滤、收集为数组、错误捕获）。
...
实现 `CanActivate` 接口，支持依赖注入 `PersonService`。返回 `true` 放行，`false` 则返回 403。
...
### 中间件 - LoggerMiddleware 与 PersonMiddleware

**LoggerMiddleware**（全局中间件）：请求前后打印日志。
...
**PersonMiddleware**（局部中间件）：演示在中间件中通过依赖注入调用 Service。
...
模块实现 `NestModule` 接口，通过 `configure` 方法将中间件绑定到指定路由。
...
### AppModule - IoC 容器全局注册

通过 `APP_GUARD`、`APP_INTERCEPTOR`、`APP_PIPE`、`APP_FILTER` 等 token 在 IoC 容器中全局注册，确保依赖注入可用。
...
@Module({
  ...
  providers: [
    AppService,
    { provide: APP_GUARD, useClass: PersonGuard },
...
### PersonController - 综合使用示例

在控制器中演示了拦截器、过滤器、管道的各种绑定方式（方法级/控制器级）。
...
export class PersonController {
  ...
  @Get('name')
  @UseInterceptors(AuthInterceptor)       // 方法级拦截器
  findName() {
    return from(['hello', 'worldA', 'abc']);
  }

  @Delete(':id')
  @UseFilters(MyExceptionFilter)          // 方法级过滤器
  remove(@Param('id', ParseIntPipe) id: string) {  // 参数级管道
    return this.personService.remove(+id);
  }
}
```
...
async function bootstrap() {
  ...
  // app.useGlobalGuards(new PersonGuard());         // 全局守卫（new方式，无法注入）
  // app.useGlobalInterceptors(new TimeoutInterceptor());
  // app.useGlobalPipes(new ParseIntPipe());
  // app.useGlobalFilters(new MyExceptionFilter());
